...
sub print_to_file {
  ...
  for ( $line = $[; $line <= $#data_scalar; $line++) {
    ...
    print FILE "$apf_counter1@$apf_counter2@$source_kks@$source_signal"
    ."@$source_description@$source_ap@$source_apf"
    ."@$destination_kks@$destination_description@$destination_ap@$destination_apf\n";
  }
  undef @data_scalar;
} 
1;